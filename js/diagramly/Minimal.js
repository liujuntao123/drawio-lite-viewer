...
EditorUi.initMinimalTheme = function()
{
	...
	EditorUi.prototype.formatEnabled = urlParams['format'] == '1';
	EditorUi.prototype.sidebarEnabled = urlParams['sidebar'] == '1';
	...
	function toggleShapes(ui, visible)
	{
		if (EditorUi.windowed)
		{
			...
			if (ui.sidebarWindow == null)
			{
				...
				ui.sidebarWindow.window.setVisible(visible === true);
	...
	EditorUi.prototype.setGraphEnabled = function(enabled)
	{
		...
		else
		{
            ...
            if (this.formatWindow != null && iw >= 1000 && this.formatEnabled &&
				this.editor.graph.getSelectionCount() > 0)
            {
	...
	EditorUi.prototype.init = function()
	{
		...
		menubar.className = 'geMenubarContainer geLegacyMinimalMenu';
        ...
        function createGroup(btns, op, container)
        {
			...
			(container || menubar).appendChild(btnGroup);
		...
		function createFloatingButton(label, tooltip, actionId, img, cssClass)
		{
			var btn = document.createElement('button');
			btn.className = 'geFloatButton' + ((cssClass != null) ? ' ' + cssClass : '');
			btn.setAttribute('type', 'button');
			btn.setAttribute('title', tooltip || label || '');
			...
			if (actionId != null)
			{
				var action = ui.actions.get(actionId);
		...
		function createShapeButton(label, tooltip, actionId, cssClass)
		{
			var btn = createFloatingButton(label, tooltip, actionId, null, cssClass);
			btn.setAttribute('data-shape', actionId);
		...
		function createChineseMenu()
		{
			...
			wrap.className = 'geFloatMenuWrap';
			var trigger = createFloatingButton('', '菜单', null, Editor.menuImage,
				'geFloatIconOnly');
			var menu = document.createElement('div');
			menu.className = 'geFloatDropdown geFloatMainDropdown';

			function addItem(label, actionId)
			{
				var item = document.createElement('button');
				item.className = 'geFloatDropdownItem';
				item.setAttribute('type', 'button');
				mxUtils.write(item, label);
				mxEvent.addListener(item, 'click', function(evt)
				{
					runAction(actionId, evt);
					wrap.classList.remove('geOpen');
					mxEvent.consume(evt);
				});
			...
			function addSeparator()
			{
				...
				sep.className = 'geFloatDropdownSep';
			...
			mxEvent.addListener(trigger, 'click', function(evt)
			{
				wrap.classList.toggle('geOpen');
				mxEvent.consume(evt);
			});

			mxEvent.addListener(document, 'click', function(evt)
			{
				if (!mxUtils.isAncestorNode(wrap, mxEvent.getSource(evt)))
				{
					wrap.classList.remove('geOpen');
				}
			});
		...
		function createFloatingShell()
		{
			...
			shell.className = 'geFloatingShell';

			var main = document.createElement('div');
			main.className = 'geFloatCluster geFloatMainMenu';
			main.appendChild(createChineseMenu());

			var tools = document.createElement('div');
			tools.className = 'geFloatCluster geFloatToolbox';
			tools.appendChild(createShapeButton('', mxResources.get('rectangle'), 'insertRectangle',
				'geFloatIconRect'));
			tools.appendChild(createShapeButton('', mxResources.get('ellipse'), 'insertEllipse',
				'geFloatIconEllipse'));
			tools.appendChild(createShapeButton('', mxResources.get('rhombus'), 'insertRhombus',
				'geFloatIconDiamond'));
			tools.appendChild(createShapeButton('', mxResources.get('line'), 'insertEdge',
				'geFloatIconLine'));
			tools.appendChild(createShapeButton('T', mxResources.get('text'), 'insertText',
				'geFloatIconText'));
			tools.appendChild(createFloatingButton('', mxResources.get('image'), 'insertImage',
				Editor.imageImage, null));
			tools.appendChild(createFloatingButton('', mxResources.get('link'), 'insertLink',
				Editor.linkImage, null));

			var zoom = document.createElement('div');
			zoom.className = 'geFloatCluster geFloatZoom';
			zoom.appendChild(createFloatingButton('-', mxResources.get('zoomOut'), 'zoomOut',
				null, null));
			zoom.appendChild(zoomElt);
			zoom.appendChild(createFloatingButton('+', mxResources.get('zoomIn'), 'zoomIn',
				null, null));

			var history = document.createElement('div');
			history.className = 'geFloatCluster geFloatHistory';
			history.appendChild(createFloatingButton('', mxResources.get('undo'), 'undo',
				Editor.undoImage, null));
			history.appendChild(createFloatingButton('', mxResources.get('redo'), 'redo',
				Editor.redoImage, null));
		...
		ui.container.appendChild(createFloatingShell());
		...
		var updateFormatWindow = mxUtils.bind(this, function()
		{
			if (EditorUi.windowed && ui.editor.graph.isEnabled())
			{
				if (ui.editor.graph.getSelectionCount() > 0)
				{
		...
		graph.getSelectionModel().addListener(mxEvent.CHANGE, updateFormatWindow);
		graph.getModel().addListener(mxEvent.CHANGE, updateFormatWindow);
		updateFormatWindow();
        ...
        function refreshMenu()
        {
			...
			var exportElt = addMenu('exportAs', true, null);

			createGroup([appElt]);
			createGroup([insertElt, exportElt]);
			...
			{
				createGroup([undoElt, redoElt, addMenuItem(mxResources.get('delete'), ui.actions.get('delete').funct,
